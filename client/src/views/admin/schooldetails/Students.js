import { cilArrowThickLeft } from '@coreui/icons'
import CIcon from '@coreui/icons-react'
import { CCard, CCardBody, CCol, CRow } from '@coreui/react'
import React, { useEffect, useState } from 'react'
import { Link } from 'react-router-dom'
import AdminHeader from 'src/components/admin/AdminHeader'

import 'src/scss/_custom.scss'

const Students = () => {

    const [student, setStudent] = useState([])
    
    useEffect(() => {
        const fetchStudent = async () => {
        const result = await fetch('http://localhost:3005/studentdata')
        const jsonStudent = await result.json()
    
        setStudent(jsonStudent)
        }
        fetchStudent()
    }, [])


  return (
    <div>
        <AdminHeader />
        <div className='add-div'>
                <Link to='/schooldetails' >
                    <CIcon icon={cilArrowThickLeft} /> Back
                </Link>
                <CRow>
                    <CCol >
                        <h2 className='text-center defaultcolor'>Students</h2>
                        <CCard className='w-75 shadow middle-card'>
                            <CCardBody>
                                {student.map(student => 
                                <div key={student.student_id} className=''>
                                    <CCardBody>{student.student_name}</CCardBody>
                                    
                                </div>
                                )}
                            </CCardBody>
                        </CCard>
                       
                    </CCol>
                </CRow>
        </div>
    </div>
  )
}

export default Students
